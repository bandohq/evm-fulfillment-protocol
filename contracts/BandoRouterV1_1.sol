...
import { FulFillmentRequest, ERC20FulFillmentRequest, FulFillmentRecord, ERC20FulFillmentRecord } from "./FulfillmentTypes.sol";
...
contract BandoRouterV1_1 is
    ...
    function withdrawERC20Refund(uint256 serviceID, uint256 recordId) public virtual {
        ERC20FulFillmentRecord memory record = IBandoERC20FulfillableV1_1(_erc20Escrow).record(recordId);
        if (msg.sender != record.payer) {
            revert PayerMismatch(record.payer, msg.sender);
        }
        IBandoERC20FulfillableV1_1(_erc20Escrow).withdrawERC20Refund(serviceID, recordId);
    }
    ...
    /// @param serviceID The service identifier
    /// @param recordId The fulfillment record identifier
    function withdrawRefund(uint256 serviceID, uint256 recordId) public virtual {
        FulFillmentRecord memory record = IBandoFulfillableV1_1(_escrow).record(recordId);
        if (msg.sender != record.payer) {
            revert PayerMismatch(record.payer, msg.sender);
        }
        IBandoFulfillableV1_1(_escrow).withdrawRefund(serviceID, recordId);
    }
}
